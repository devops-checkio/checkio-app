...
interface AssistanceIncompleteProps {
  filters?: any;
}

const AssistanceIncomplete = ({ filters }: AssistanceIncompleteProps) => {
  ...
  const handleOpenModal = useCallback((employeeId: string) => {
    setSelectedRowKeys([employeeId]);
    setIsModalOpen(true);
  }, []);
  ...
  }, [pagination.current, pagination.pageSize, pagination.sort, filters]);
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        >
          <div className="flex gap-3">
            {canCreateAssistance && (
              <CHEKIOButton variant="primary" onClick={() => setIsModalCreatorOpen(true)}>
          ...
          {isLoading ? (
                ...
                <CHEKIOTable className="rounded-none border-0 shadow-none">
                  <CHEKIOTableHeader>
                    <tr>
                      ...
                      <TableHeaderCell icon={Camera} label="Verificación Fotos" />
                      <CHEKIOTableHead className="min-w-[100px] text-right">Acciones</CHEKIOTableHead>
                    </tr>
                  </CHEKIOTableHeader>
                  <CHEKIOTableBody>
            ...
            <div className="flex flex-col items-center justify-center px-6 py-24">
              ...
              <p className="mt-1 max-w-sm text-center text-sm text-gray-500">
                No se encontraron asistencias incompletas para los filtros seleccionados.
              </p>
            </div>
          ) : (
            <>
              ...
              <CHEKIOTable className="rounded-none border-0 shadow-none">
                ...
                <CHEKIOTableBody>
                  {assistanceData.map((assistance, index) => {
                    ...
                    // Entrada: solo entrada a turno = CHECK_IN sin scheduleBreakId
                    const markStart =
                      marks.find(
                        (m: any) =>
                          m.type === "CHECK_IN" &&
                    ...
                    // Colaciones: las dos marcas de colación (entrada y salida a colación = con scheduleBreakId), ordenadas por timestamp
                    ...
                    return (
                        <CHEKIOTableRow key={assistance.publicId} index={index}>
                          ...
                          <CHEKIOTableCell className="px-5 py-3.5">{date}</CHEKIOTableCell>
...
export default AssistanceIncomplete;
