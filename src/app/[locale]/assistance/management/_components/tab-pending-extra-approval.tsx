...
import { useGetAssistancesPendingExtraApproval } from "@/service/mantainer.service";
...
import { AssistanceExtraApproveModal } from "./assistance-extra-approve-modal";
import { AssistanceExtraRejectModal } from "./assistance-extra-reject-modal";
...
interface TabPendingExtraApprovalProps {
  filters?: Record<string, unknown>;
}

export default function TabPendingExtraApproval({
  filters = {},
}: TabPendingExtraApprovalProps) {
  const t = useTranslations("assistanceManagement.tabs.extraApproval");
  ...
  const { data, isLoading } =
    useGetAssistancesPendingExtraApproval(queryParams);
              ...
              <CHEKIOTable className="rounded-none border-0 shadow-none">
                <CHEKIOTableHeader>
                  <tr>
                    ...
                    <TableHeaderCell icon={Clock} label={t("headers.extraSeconds")} />
            ...
            <CHEKIOTableBody>
              {list.map((item: AssistanceResponseDto, index: number) => (
                <CHEKIOTableRow key={item.publicId} index={index}>
                  ...
                  <CHEKIOTableCell className="px-5 py-3.5">
                    {formatSecondsToHoursMinutes(item.extraSeconds ?? 0)}
                  </CHEKIOTableCell>
                  <CHEKIOTableCell className="px-5 py-3.5">
