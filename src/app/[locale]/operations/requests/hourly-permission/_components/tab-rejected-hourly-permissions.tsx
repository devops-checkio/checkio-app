...
import { useGetRejectedHourlyPermissions } from "@/service/hourly-permission.service";
import { CalendarX2, ChevronLeft, ChevronRight, Eye } from "lucide-react";
...
export default function TabRejectedHourlyPermissions() {
  const t = useTranslations("operations.requests.hourlyPermission.tabs.rejected");
  ...
  const { data: rejectedRequestsData, isLoading } =
  ...
  const data = rejectedRequestsData?.data || [];
  const pagination = rejectedRequestsData?.pagination || {
    current: 1,
    totalPages: 1,
    totalCount: 0,
    pageSize: 10,
            ...
            <CHEKIOTable className="w-full rounded-none border-0 shadow-none">
              <CHEKIOTableHeader>
                <tr>
                  ...
                  <CHEKIOTableHead>{t("table.headers.rejectedBy")}</CHEKIOTableHead>
                  <CHEKIOTableHead>{t("table.headers.rejectedAt")}</CHEKIOTableHead>
                  <CHEKIOTableHead>{t("table.headers.rejectionReason")}</CHEKIOTableHead>
            ...
            <CHEKIOTableBody>
              {data.map((request: HourlyPermissionResponseDto, index: number) => (
                <CHEKIOTableRow key={request.publicId} index={index}>
                  ...
                  <CHEKIOTableCell>
                    {request.rejectedByName || request.rejectedBy || "-"}
                  </CHEKIOTableCell>
                  <CHEKIOTableCell>
                    {request.rejectedAt
