  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <h4 className="text-sm font-medium text-gray-900">{t("holiday")}</h4>
      </div>

      <div className="space-y-1">
        <div
          className={`flex items-center justify-center gap-2 rounded-lg p-1.5 min-w-[120px] ${
            isSelected ? "bg-blue-100" : "bg-red-50"
          }`}
        >
          <span className="text-xs font-medium">00:00</span>
          <span className="text-xs text-red-500">{t("until")}</span>
          <span className="text-xs font-medium">23:59</span>
        </div>

        <div className="text-xs text-red-500">{t("holidayDay")}</div>
      </div>
    </div>
  );
};
