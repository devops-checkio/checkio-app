...
/**
 * Generates a circular polygon GeoJSON geometry
 */
function createCircle(
  ...
  const lngRadius =
    radiusInMeters / (earthRadius * Math.cos((Math.PI * lat) / 180));

  const coordinates: [number, number][] = [];
  for (let i = 0; i <= steps; i++) {
    const angle = (i / steps) * 2 * Math.PI;
    const dx = Math.cos(angle) * lngRadius;
    const dy = Math.sin(angle) * latRadius;
    coordinates.push([lng + dx * (180 / Math.PI), lat + dy * (180 / Math.PI)]);
  }
...
type MapCircleProps = {
  ...
  id?: string;
  /** Longitude coordinate for circle center */
  longitude: number;
  /** Latitude coordinate for circle center */
  latitude: number;
  /** Radius in meters (default: 100) */
  radius?: number;
  /** Fill color as CSS color value (default: "#DCDCBCFF") */
  fillColor?: string;
  /** Fill opacity from 0 to 1 (default: 0.35) */
  fillOpacity?: number;
  /** Stroke color as CSS color value (default: "#DCDCBCFF") */
  strokeColor?: string;
  /** Stroke width in pixels (default: 2) */
  strokeWidth?: number;
  /** Stroke opacity from 0 to 1 (default: 0.8) */
  strokeOpacity?: number;
  /** Whether the circle is interactive - shows pointer cursor on hover (default: false) */
  interactive?: boolean;
  /** Callback when the circle is clicked */
  onClick?: () => void;
  /** Callback when mouse enters the circle */
  onMouseEnter?: () => void;
  /** Callback when mouse leaves the circle */
  onMouseLeave?: () => void;
};
...
function MapCircle({
  ...
  const autoId = useId();
  const id = propId ?? autoId;
  const fillSourceId = `circle-fill-source-${id}`;
  const fillLayerId = `circle-fill-layer-${id}`;
  const strokeSourceId = `circle-stroke-source-${id}`;
  ...
  // Update circle geometry when center or radius changes
  useEffect(() => {
    if (!isLoaded || !map) return;

    const fillSource = map.getSource(fillSourceId) as MapLibreGL.GeoJSONSource;
    const strokeSource = map.getSource(
      strokeSourceId
    ) as MapLibreGL.GeoJSONSource;

    if (fillSource && strokeSource) {
      const updatedCircle = createCircle(center, radius);
      fillSource.setData({
        type: "Feature",
        properties: {},
        geometry: updatedCircle,
      });
  ...
  }, [isLoaded, map, fillSourceId, strokeSourceId, center[0], center[1], radius]);
  ...
  }, [isLoaded, map, fillLayerId, fillColor, fillOpacity]);

  useEffect(() => {
    if (!isLoaded || !map || !map.getLayer(strokeLayerId)) return;
  ...
  }, [isLoaded, map, strokeLayerId, strokeColor, strokeWidth, strokeOpacity]);
